...
pub struct YamlSer<'a, W: Write> {
    ...
    /// Threshold for scientific notation. When Some(threshold), numbers >= threshold
...
impl<'a, W: Write> YamlSer<'a, W> {
    ...
    fn write_quoted(&mut self, s: &str) -> Result<()> {
        ...
        // Internal single quotes are escaped by doubling (' → '')
        let use_single_quotes = !needs_escaping && !looks_like_other_type && !self.prefer_double_quotes;
...
impl<'a, 'b, W: Write> Serializer for &'a mut YamlSer<'b, W> {
    ...
    fn serialize_str(self, v: &str) -> Result<()> {
        ...
        // Check for non-BMP Unicode characters (codepoints >= U+10000) like emoji.
        // Go yaml.v2 uses double-quoted style for such content to output \U escapes.
        let has_non_bmp = v.chars().any(|c| (c as u32) >= 0x10000);
        ...
        // Also skip if content has non-BMP Unicode characters (like emoji) - Go yaml.v2 uses
        // double-quoted style for such content to output \U escapes.
        let has_non_bmp = v.chars().any(|c| (c as u32) >= 0x10000);
        ...
        if let Some(max_width) = self.line_width {
            ...
            // Also use quoted wrap for strings with non-BMP characters (like emoji).
            let has_non_bmp_chars = v.chars().any(|c| (c as u32) >= 0x10000);
            ...
            if self.in_flow == 0 && needs_quoted_wrap {
                ...
                let content_length = v.chars().count();
                ...
                if should_wrap && max_width > 0 {
                    ...
                    if !is_plain_block_value_safe(v) {
                        // String needs quoting - use write_quoted_with_wrap which handles
