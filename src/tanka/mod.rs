...
use self::options::{FOLDED_WRAP_CHARS, MIN_FOLD_CHARS};
...
pub use self::options::{ChompIndicator, SerializerOptions};
use self::quoting::{is_plain_block_value_safe, is_plain_safe, is_plain_value_safe};
...
///
/// This type implements `serde_core::Serializer` and writes YAML to a `fmt::Write`.
...
impl<'a, W: Write> YamlSer<'a, W> {
    ...
    fn write_quoted(&mut self, s: &str) -> Result<()> {
        ...
        let looks_like_other_type = s.is_empty()
            ...
            || self::quoting::looks_like_date_or_timestamp(s);
