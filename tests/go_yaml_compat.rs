...
fn test_single_line_with_tabs_quoted() {
    ...
    // Should use quoted style for tab character
    assert!(
...
fn test_long_quoted_string_with_tabs_and_newlines_wrapped() {
    ...
    // Should NOT use block scalar style (tabs prevent it)
    assert!(
        !buf.contains("|"),
...
fn test_double_quoted_wrap_escapes_only_first_leading_space() {
    ...
    let mut data: HashMap<&str, &str> = HashMap::new();
    data.insert("config", string_with_triple_space);
    ...
    // The line break provides 1 space (via YAML line folding), so for 3 original spaces:
    // - fold: 1 space
...
fn test_double_quoted_wrap_no_trailing_spaces() {
    ...
    // Check that no line ends with trailing spaces (before the newline)
